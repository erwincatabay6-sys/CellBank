...
function TechnicianPage() {
  ...
  const techniciansWithWorkload = mockTechnicians.map((technician) => {
  ...
  const filteredTechnicians = techniciansWithWorkload.filter((technician) => {
    const search = searchTerm.trim().toLowerCase();

    return (
      technician.name.toLowerCase().includes(search) ||
      technician.status.toLowerCase().includes(search)
    );
  });
      ...
      <section className="page-content">
        ...
        {filteredTechnicians.length > 0 ? (
          ...
          <div className="workspace-empty-state">
            ...
            <p>Try another technician search.</p>
          </div>
        )}
      </section>
    </>
