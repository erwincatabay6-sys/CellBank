...
function DashboardPage({ currentRoles, currentUserName }) {
  ...
  const isTechnician = hasRole(currentRoles, "TECHNICIAN");

  const isFrontDesk = hasRole(currentRoles, "FRONT_DESK");

  // Technician-only dashboard behavior.
  // Technician + Front Desk uses the
  // shop-wide Front Desk operational view.
  const isTechnicianOnly = isTechnician && !isFrontDesk && !isAdmin;
  ...
  if (isAdmin || isFrontDesk) {
    dashboardRepairs = mockRepairs;
  } else if (isTechnicianOnly) {
    dashboardRepairs = mockRepairs.filter(
      (repair) => repair.technician === currentUserName,
    );
  }
  ...
  const totalRepairs = dashboardRepairs.length;

  const activeRepairs = dashboardRepairs.filter(
    (repair) => !terminalStatuses.includes(repair.status),
  ).length;

  const readyForRelease = dashboardRepairs.filter(
    (repair) => repair.status === "READY_FOR_RELEASE",
  ).length;

  const completedRepairs = dashboardRepairs.filter(
  ...
  const statusCounts = dashboardRepairs.reduce((counts, repair) => {
  ...
  const attentionRepairs = dashboardRepairs.filter((repair) =>
    attentionStatuses.includes(repair.status),
  );
  ...
  const technicianWorkload = mockTechnicians.map((technician) => {
    const assignedRepairs = mockRepairs.filter(
      (repair) => repair.technicianId === technician.id,
    );

    const activeAssignedRepairs = assignedRepairs.filter(
      (repair) => !terminalStatuses.includes(repair.status),
    );
    ...
    return {
      ...
      activeRepairs: activeAssignedRepairs.length,
  ...
  if (isAdmin || isFrontDesk) {
    visibleTechnicianWorkload = technicianWorkload;
  } else if (isTechnicianOnly) {
    visibleTechnicianWorkload = technicianWorkload.filter(
      (technician) => technician.name === currentUserName,
    );
  }
  ...
  const recentActiveRepairs = dashboardRepairs
    .filter((repair) => !terminalStatuses.includes(repair.status))
    .slice(0, 5);
  ...
  const totalAgreedValue = isAdmin
    ? mockRepairs.reduce(
        (total, repair) => total + (repair.agreedPrice ?? 0),
        0,
      )
    : 0;

  const totalCollected = isAdmin
    ? mockPayments.reduce((total, payment) => total + payment.amount, 0)
    : 0;

  const outstandingBalance = isAdmin
    ? mockRepairs.reduce((total, repair) => {
        if (repair.agreedPrice == null) {
          return total;
        }

        const repairPaid = mockPayments
          .filter((payment) => payment.repairId === repair.id)
          .reduce((paymentTotal, payment) => paymentTotal + payment.amount, 0);

        const repairBalance = Math.max(repair.agreedPrice - repairPaid, 0);

        return total + repairBalance;
      }, 0)
    : 0;
  ...
  return (
    <>
      ...
      <section className="page-content">
        ...
        <DashboardStatusCounts statusCounts={statusCounts} />
      </section>
      ...
      <section className="page-content">
        ...
        <DashboardTechnicianWorkload technicians={visibleTechnicianWorkload} />
      </section>
