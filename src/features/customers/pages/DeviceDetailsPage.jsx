...
function DeviceDetailsPage() {
  ...
  const { customers, updateDevice } = useCustomers();
  ...
  const repairHistory = device
    ? mockRepairs.filter((repair) => repair.deviceId === device.id)
    : [];
  ...
  const previousRepairContext = repairHistory.flatMap(
    (repair) => repair.previousRepairs ?? [],
  );
  ...
  const problemCounts = previousRepairContext.reduce(
    ...
    return (
      <>
        <section className="page-header">
          ...
          <p>The requested device record does not exist.</p>
        </section>
  ...
  function handleDeviceUpdate(updatedDevice) {
    updateDevice(customer.id, device.id, updatedDevice);
  ...
  return (
    <>
      ...
      <section className="page-header">
        ...
        <p>Device registered under {customer.name}.</p>
      </section>
      ...
      <section className="page-content">
        ...
        <div className="device-info-grid">
          ...
          <div>
            ...
            <strong>{device.imei ?? "Not recorded"}</strong>
          </div>
          ...
          <div>
            ...
            <strong>{device.serialNumber ?? "Not recorded"}</strong>
          </div>
          ...
          <div>
            ...
            <strong>{device.notes ?? "None recorded"}</strong>
          </div>
        </div>
      </section>
      ...
      <section className="page-content">
        ...
        {repeatedProblems.length > 0 && (
          <div className="device-repeat-alert">
            <div>
              ...
              <p>
                Previous repair records show recurring problems for this device.
              </p>
            </div>

            <div className="device-repeat-list">
              {repeatedProblems.map(([problem, count]) => (
                <span key={problem}>
                  {problem}: {count} previous repairs
                </span>
              ))}
            </div>
          </div>
        ...
        {repairHistory.length > 0 ? (
          ...
          <div className="workspace-empty-state">
            ...
            <p>Repair jobs created for this device will appear here.</p>
          </div>
        )}
      </section>
