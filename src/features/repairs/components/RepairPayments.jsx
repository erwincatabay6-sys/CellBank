...
function RepairPayments({
  ...
  const canRecordPayments = hasAccess(currentRoles, "recordPayments");
  ...
  const [payments, setPayments] = useState(() =>
    mockPayments.filter((payment) => payment.repairId === repairId),
  );
  ...
  useEffect(() => {
    const repairPayments = mockPayments.filter(
      (payment) => payment.repairId === repairId,
    );
  ...
  const effectiveRepairTotal = repairTotal ?? 0;
  ...
  const balance = Math.max(effectiveRepairTotal - totalPaid, 0);

  let paymentStatus = "Unpaid";

  if (repairTotal == null) {
    paymentStatus = "Price Not Agreed";
  } else if (totalPaid >= effectiveRepairTotal && effectiveRepairTotal > 0) {
    paymentStatus = "Paid";
  } else if (totalPaid > 0) {
    ...
    <section className="page-content repair-payments">
      ...
      <div className="payment-summary">
        ...
        <div>
          ...
          <strong>
            {repairTotal != null ? `₱${balance.toFixed(2)}` : "—"}
          </strong>
        </div>
          ...
          {payments.map((payment) => (
            <article key={payment.id} className="payment-item">
              <div className="payment-item-top">
                ...
                <span>{payment.recordedAt}</span>
              </div>

              <p>
                Recorded by <strong>{payment.recordedBy}</strong>
              </p>
