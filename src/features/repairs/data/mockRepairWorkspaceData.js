...
export const mockFindingsByRepairId = {
  1: [
    {
      id: 101,
      finding: "Charging port shows signs of wear and intermittent contact.",
      diagnosis: "Possible damaged charging port.",
      actionTaken: "Charging port inspected and connection tested.",
      recordedBy: "Miguel Santos",
    },
    {
      id: 102,
      finding: "Battery health is within acceptable range.",
      diagnosis: "Battery is unlikely to be the primary cause.",
      actionTaken: "Battery health test completed.",
      recordedBy: "Miguel Santos",
    },
  ],
  ...
  2: [
    {
      id: 201,
      finding: "Cooling vents and fan assembly contain heavy dust buildup.",
      diagnosis: "Restricted airflow is contributing to overheating.",
      actionTaken: "Cooling system inspected and prepared for cleaning.",
      recordedBy: "Carlo Mendoza",
    },
  ],
  ...
  3: [
    {
      id: 301,
      finding: "Display glass is cracked and touch response is intermittent.",
      diagnosis: "Display assembly requires replacement.",
      actionTaken: "Replacement display installed and touch response tested.",
      recordedBy: "Miguel Santos",
    },
  ],
  ...
  4: [
    {
      id: 401,
      finding: "Battery capacity is below normal operating range.",
      diagnosis: "Battery degradation caused the reported rapid drain.",
      actionTaken: "Battery replaced and charging cycle tested.",
      recordedBy: "Miguel Santos",
    },
  ],
...
export function getMockFindings(repairId) {
  return [...(mockFindingsByRepairId[repairId] ?? [])];
}

export function getMockStatusHistory(repairId) {
  return [...(mockStatusHistoryByRepairId[repairId] ?? [])];
}

export function getMockParts(repairId) {
  return [...(mockPartsByRepairId[repairId] ?? [])];
}
