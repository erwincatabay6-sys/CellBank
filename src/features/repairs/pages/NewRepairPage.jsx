...
function getDeviceLabel(device) {
  if (device.imei) {
    return (
      `${device.brand} ${device.model}` + ` — IMEI ••••${device.imei.slice(-4)}`
    );
  }
...
function NewRepairPage() {
  ...
  const { customers, addCustomer, addDevice } = useCustomers();
  ...
  const selectedDevicePreviousRepairs = selectedDevice
    ? mockRepairs.filter((repair) => repair.deviceId === selectedDevice.id)
        .length
    : 0;
  ...
  function handleNewCustomer(newCustomer) {
    const registeredCustomer = addCustomer(newCustomer);

    setSelectedCustomerId(String(registeredCustomer.id));
  ...
  function handleNewDevice(newDevice) {
    ...
    const registeredDevice = addDevice(selectedCustomer.id, newDevice);

    setSelectedDeviceId(String(registeredDevice.id));
  ...
  function handleSubmit(event) {
    ...
    setTimeout(() => {
      ...
      setSubmitMessage("Repair information validated successfully.");
    }, 700);
  }
  ...
  return (
    <>
      <section className="page-header">
        ...
        <p>Create a new Cellbank repair job.</p>
      </section>
      ...
      <form className="repair-form" onSubmit={handleSubmit}>
        ...
        <section className="repair-form-section">
          <div className="repair-form-section-header">
            <div>
              ...
              <p>
                Select one of the customer's registered devices or register a
                new device.
              </p>
            </div>
          ...
          {selectedDevice && (
            <div className="selected-device-summary">
              ...
              <div className="selected-device-grid">
                ...
                <div>
                  <span>IMEI</span>
                  <strong>{selectedDevice.imei ?? "Not recorded"}</strong>
                </div>

                <div>
                  <span>Previous Repairs</span>
                  <strong>{selectedDevicePreviousRepairs}</strong>
                </div>

                <div>
                  <span>Notes</span>
                  <strong>{selectedDevice.notes ?? "Not recorded"}</strong>
                </div>
              </div>
            </div>
          )}
        ...
        <section className="repair-form-section">
          <div className="repair-form-section-header">
            <div>
              ...
              <p>Record the reported issue and repair intake details.</p>
            </div>
          </div>

          <div className="repair-form-grid">
            <div className="repair-form-group repair-form-full">
              <label htmlFor="reported-problem">Reported Problem</label>
            ...
            <div className="repair-form-group">
              <label htmlFor="accessories-received">Accessories Received</label>
        ...
        <section className="repair-form-section">
          <div className="repair-form-section-header">
            <div>
              ...
              <p>
                Optionally assign a technician and record an initial estimate.
              </p>
            </div>
          </div>
        ...
        {submitMessage && (
          <div className="repair-submit-message">{submitMessage}</div>
        )}
        ...
        <div className="repair-form-actions">
          ...
          >
            {isSubmitting ? "Creating Repair..." : "Create Repair"}
          </button>
        </div>
      </form>
